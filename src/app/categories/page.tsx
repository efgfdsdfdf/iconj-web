...
export default async function CategoriesPage() {
  ...
  // Fetch real categories and their custom admin uploaded images
  ...
  const adminCategories: { name: string, icon: string, description?: string }[] = settings?.value || [];
  
  // We will base the page PRIMARILY on adminCategories since the admin explicitly set these up with images
  // We'll try to find the matching category ID from the DB to pass to the shop filter.
  // If no DB match is found, we'll just pass the name.
  
  let categoriesToDisplay: any[] = [];

  if (adminCategories.length > 0) {
    categoriesToDisplay = adminCategories.map(ac => {
    ...
    categoriesToDisplay = (dbCategories || []).map(cat => ({
      title: cat.name,
      id: cat.id,
