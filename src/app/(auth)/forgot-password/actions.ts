...
export async function resetPassword(formData: FormData) {
  ...
  // Build our own direct reset URL using the hashed_token — bypasses Supabase redirect whitelist
  const hashedToken = data?.properties?.hashed_token;
  if (hashedToken) {
    const resetUrl = `https://iconj.com.ng/api/auth/callback?token_hash=${hashedToken}&type=recovery&next=/update-password`;
    ...
    const htmlEmail = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background: #f8fafc; padding: 24px; border-radius: 12px;">
        ...
        <div style="background: white; padding: 28px; border-radius: 0 0 8px 8px; border: 1px solid #e2e8f0; border-top: none;">
          ...
          <p style="color: #475569; margin: 0 0 24px; line-height: 1.6;">We received a request to reset your password for your ICONJ account. If you did not make this request, you can safely ignore this email.</p>
